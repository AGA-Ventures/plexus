-- Vendor acceptance is intentionally one-way. Admins may still reset a match
-- to Proposed or Rejected, but a Vendor can no longer reject a match or clear
-- its own acceptance after recording it. A legacy Rejected row can be reopened
-- only by recording the Vendor's acceptance while returning the derived status
-- to Proposed (or Accepted when the counterparty already accepted).
  ...
  if tg_op = 'UPDATE' and actor_role = 'vendor' then
    ...
    then
      ...
      then
        raise exception using
          errcode = '42501',
          message = 'Vendor acceptance cannot be withdrawn';
