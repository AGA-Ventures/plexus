  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      ...
      if (result.ok) {
        ...
        toast.error(result.error ?? "Unable to update the workspace.")
      }
    })
  }
    ...
    <form className="grid gap-4" onSubmit={submit}>
      <div className="grid gap-3 sm:grid-cols-2">
        <div className="grid gap-1.5">
          <Label htmlFor={`tenantName-${tenantId}`}>Workspace name</Label>
          <Input
      ...
      <div className="grid gap-1.5">
        ...
        <div className="flex items-center gap-3">
          ...
          <p className="text-xs text-muted-foreground">
            Applied to buttons, highlights, and the tenant login.
          </p>
        </div>
      </div>
      <p className="text-xs text-muted-foreground">
        Save name, support, color, or manual URL changes before previewing them.
      </p>
      <DialogFooter className="gap-2 sm:justify-between">
        ...
        <Button type="submit" disabled={pending || uploading}>
          ...
          {pending ? "Saving…" : "Save white label"}
        </Button>
      </DialogFooter>
    </form>
  )
